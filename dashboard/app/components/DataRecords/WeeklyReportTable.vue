<template>
  <div class="w-full mt-6 px-2 md:px-0 max-w-[1250px] mx-auto pb-20 font-sans text-slate-900">
    ...
    <div class="bg-white p-6 rounded-[1rem] border border-slate-100 shadow-sm mb-6 transition-all">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-5">
        <div class="lg:col-span-4">
          <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 ml-1 tracking-wider">Pencarian Cepat</label>
          <input type="text" v-model="filters.search" @input="handleSearch" placeholder="Cari ID Objek, Jenis Infrastruktur..." class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-semibold" />
        </div>

        <div class="lg:col-span-2">
          <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 ml-1 tracking-wider">Tanggal</label>
          <input type="date" v-model="filters.date" @change="handleFilterChange" class="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm focus:ring-4 focus:ring-blue-500/10 outline-none font-semibold" />
        </div>

        <div class="lg:col-span-3">
          <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 ml-1 tracking-wider">Pengamat</label>
          <select v-model="filters.id_karyawan" @change="handleFilterChange" class="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm outline-none font-semibold appearance-none">
        ...
        <div class="lg:col-span-3">
          <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 ml-1 tracking-wider">Lokasi</label>
          <SearchableSelect 
    ...
    <transition enter-active-class="transition duration-300 ease-out" enter-from-class="transform -translate-y-4 opacity-0" enter-to-class="transform translate-y-0 opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="transform translate-y-0 opacity-100" leave-to-class="transform -translate-y-4 opacity-0">
        <div v-if="selectedIds.length > 0" class="mb-6 flex items-center justify-between p-5 bg-red-600 rounded-[2rem] shadow-2xl shadow-red-200 text-white">
            <div class="flex items-center gap-3">
                <div class="bg-white/20 p-2 rounded-xl"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></div>
                <span class="text-xs font-black uppercase tracking-widest">{{ selectedIds.length }} Data Terpilih</span>
            </div>
            <div class="flex gap-2">
                <button @click="bulkDelete" class="px-6 py-2.5 bg-white text-red-600 text-[10px] font-black rounded-xl hover:bg-red-50 transition uppercase tracking-widest">Hapus Permanen</button>
                <button @click="selectedIds = []" class="px-6 py-2.5 bg-red-700 text-white text-[10px] font-black rounded-xl hover:bg-red-800 transition uppercase tracking-widest border border-red-500">Batal</button>
            </div>
        </div>
    </transition>
    ...
    <div class="bg-white border border-slate-100 rounded-[1rem] shadow-2xl shadow-slate-200/40 overflow-hidden">
      ...
      <div class="block md:hidden bg-slate-50/50 p-3">
        <div v-if="isLoading" class="py-20 text-center font-bold text-blue-500 animate-pulse text-xs uppercase tracking-widest">Sinkronisasi Server...</div>
        <div v-for="item in responseData.data" :key="item.id" class="bg-white p-6 mb-4 rounded-[2rem] border border-slate-100 shadow-sm relative transition-all active:scale-[0.98]">
          <div class="flex justify-between items-start mb-6">
            <div class="flex gap-4">
              <input type="checkbox" v-model="selectedIds" :value="item.id" class="mt-1 rounded-lg border-slate-200 text-blue-600 h-6 w-6" />
              <div>
      ...
      <div v-if="responseData.total > 0" class="px-10 py-8 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div class="flex gap-3">
          ...
          <button @click="changePage(responseData.current_page + 1)" :disabled="responseData.current_page === responseData.last_page" class="px-6 py-3 bg-white border border-slate-200 rounded-2xl text-[10px] font-black text-slate-600 hover:bg-slate-100 disabled:opacity-30 transition uppercase tracking-widest">Next</button>
        </div>
      </div>
    </div>
  </div>
...
const ScoreBadge = ({ skor }) => {
  if (!skor || skor === 0) return h('div', { class: 'text-[9px] font-black text-slate-300 bg-slate-100 px-2 py-0.5 rounded-lg' }, 'N/A')
  const theme = { 3: 'bg-emerald-500 text-white', 2: 'bg-orange-500 text-white', 1: 'bg-red-500 text-white' }
  return h('div', { class: `text-[10px] font-black px-2 py-0.5 rounded-lg ${theme[skor] || 'bg-slate-200'}` }, skor)
}

const isLoading = ref(false), selectedIds = ref([])
...
const mappedLokasi = computed(() => options.value.lokasi.map(loc => ({ id: loc.id, display: `${loc.afdeling} - ${loc.blok}` })))
...
const bulkDelete = async () => {
    const res = await Swal.fire({
        ...
        text: `Hapus ${selectedIds.value.length} data?`,
        icon: 'warning',
